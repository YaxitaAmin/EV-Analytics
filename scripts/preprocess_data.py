...
def preprocess_ev_stations(input_file, output_file):
    """
    Preprocess EV charging station data from AFDC dataset.
    """
    # Read the dataset with all columns as string initially
    df = pd.read_csv(input_file, low_memory=False)
    
    # Select only EV-relevant columns
    columns_to_keep = [
    ...
    # Create accessibility features - fixed the data type issues
    ...
    # Create connector type features
    ...
    # Calculate station density score (0-1 scale)
    df['station_density_score'] = (df['stations_in_state'] / df['stations_in_state'].max()).round(3)
    
    # Create composite accessibility score (0-1 scale)
    ...
    # Simulate utilization metrics (for analysis purposes)
